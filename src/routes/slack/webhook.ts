...
export const webhookRoute = async (c: Context) => {
  // Read the raw body bytes for HMAC signature verification
  const rawBodyBuffer = await c.req.arrayBuffer();
  const bodyString = new TextDecoder().decode(rawBodyBuffer);
  ...
  if (signingSecret) {
    ...
    const receivedSig = c.req.header("x-slack-signature") || "";
